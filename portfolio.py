...
def make_request(portfolio: dict, currency: str, refetch: int, show_btc: bool, show_eth: bool):
    ...
    if (show_btc or show_eth): # fetch btc or eth values if the user wants to but doesn't have any.
    ...
    for key in response:
        print(f"{key[:14]:<15}{response[key][currency]:>10}{portfolio[key]:>20}{round(response[key][currency] * portfolio[key], 2):>20}")
...
if __name__ == "__main__":
    ...
    while True:
        make_request(portfolio, currency, refetch, show_btc, show_eth)
        time.sleep(refetch)
        os.system(clear)
